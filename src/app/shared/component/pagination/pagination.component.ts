...
export class PaginationComponent implements OnInit, OnChanges {
  ...
  ngOnChanges() {
    this.pages = this.createPageArray(this.pagedResult && +this.pagedResult.currentPage + 1,
      this.pagedResult && +this.pagedResult.pageSize, this.pagedResult && +this.pagedResult.total, this.maxSize);
  }
  ...
  getPage(page: number): void {
    ...
    if (page == Number.parseInt(this.pagedResult.pageCount.toString())) {
      return;
    }
    if (page == 0 && this.pagedResult.currentPage == 0) {
      return;
    }
    if (page == Number.parseInt(this.pagedResult.pageCount.toString()) - 1
      && this.pagedResult.currentPage == Number.parseInt(this.pagedResult.pageCount.toString()) - 1) {
      return;
    }
